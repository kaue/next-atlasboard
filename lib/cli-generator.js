...
function dir_has_atlas_project(dir){
    var requiredItems = ["packages", "package.json"]; //the proyect should have these items
...
var commands = {
    ...
    generate: function (project_dir, default_package, item_type, item_name, callback){
        ...
        var newItemBaseLocation = path.join(project_dir, "packages", default_package);
        var newItemLocation = path.join(newItemBaseLocation, item_type + "s", item_name);
...
// ----------------------------------
if (command == "generate") {
    return commands.generate(process.cwd(), "default", first, second, function(err){
        if (err){
            console.error(err);
        }
    });
