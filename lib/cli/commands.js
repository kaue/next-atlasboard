...
// ----------------------------------
if (command == "generate") {
  var package_folder = "default";
  var item_name = second;
  if (second.indexOf('#') > -1){ //package namespacing
    package_folder = second.split('#')[0];
    item_name = second.split('#')[1];
  }
  return logic_cli.generate(process.cwd(), package_folder, first, item_name, function(err){
    if (err){
      console.error(err);
    }
  });
