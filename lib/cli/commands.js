...
module.exports = {
  ...
  start: {
    ...
    example: "atlasboard start 3333   #runs atlasboard in port 3333" +
             "\n        atlasboard start 3333 --jobFilter myjob   #runs only executing jobs matching 'myjob'" +
             "\n        atlasboard start 3333 --dashboardFilter \\bdash   #only loads dashboards matching \\bdash regex",

    run: function (args, cb) {
           var port = isNaN(args[0]) ? 3000 : args[0];
           var options = {port: port, filters: {}};

           var argsOptimistic = require('optimist')(args).argv;
           
           if (argsOptimistic.jobFilter) {
             logger.log('Loading jobs matching ' + argsOptimistic.jobFilter + " only");
             options.filters.jobFilter = argsOptimistic.jobFilter;
           }

           if (argsOptimistic.dashboardFilter) {
             logger.log('Loading dashboards matching ' + argsOptimistic.dashboardFilter + " only");
             options.filters.dashboardFilter = argsOptimistic.dashboardFilter;
           }
