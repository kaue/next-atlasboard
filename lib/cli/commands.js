    ...
    run: function (args, cb) {
           ...
           return logicCli.newProject (srcDir, destDir, function(err){
             ...
             child.on('exit', function () {
               console.log('\nSUCCESS !!');
               console.log('\nNew project "%s" successfully created. Now you can:\n', newDirectory);
               console.log(' 1. cd ' + newDirectory);
               console.log(' 2. npm install');
               console.log(' 3. browse http://localhost:3000');
               console.log(' 4. start your server with `atlasboard start`');
               console.log(' \noptionally: import the Atlassian package (or any other package) by running:');
               console.log(' \n\tgit init');
               console.log(' \tgit submodule add https://bitbucket.org/atlassian/atlasboard-atlassian-package packages/atlassian\n');
               cb && cb();
             });
          });
        }
