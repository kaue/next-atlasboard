...
module.exports = {
    ...
    renderScriptAssets : function (atlasboard_assets_folder, wallboard_assets_folder, req, res){
      ...
      var atlasboard_js = ["jquery-1.8.3.min", "underscore-1.4.4.min",
          "console-helper", "jquery.gridster-0.1.0.min", "application"].map(function(file){
              return path.join(atlasboard_assets_folder, '/javascripts/' + file + '.js'); });

      res.type("application/javascript");
      res.write("var Widgets = {};\n\n");

      atlasboard_js.forEach(function(file){
        res.write(fs.readFileSync(file) + "\n\n");
      });
    ...
    renderCssGeneralAssets : function (atlasboard_assets_folder, req, res){
      ...
      stylus(stylus_str, { filename: "application.styl" }).use(require("nib")()).render(function(err, css){
        if (err) throw err;
        res.write(css);
        res.end();
      });
