    ...
    parsed_namespacedCss.stylesheet.rules.forEach(function(rule) {
      rule.selectors.forEach(function (selector) {
        if (!rule.declarations){
          logger.error('Error. No CSS declaration for selector ' + selector + ' in widget ' + widget_namespace); // don't put comments within the rules!
        }
        else {
...
module.exports = {
    ...
    render_dashboard : function (packagesPath, dashboard_name, req, res){
      ...
      item_manager.get_first(packagesPath, dashboard_name, "dashboards", ".json", function(err, dashboard){
        if (err || !dashboard){
          return res.send(err ? 400 : 404, err ? err : "Trying to render dashboard " + dashboard_name + ", but couldn't find any dashboard in the packages folder");
        }
    ...
    // Render JS
    // ---------------------------------------------------------------
    render_script_assets : function (atlasboard_assets_folder, wallboard_assets_folder, req, res){
    ...
    // ---------------------------------------------------------------
    js_widget: function (packagesPath, widget_name, req, res){
      res.type("application/javascript");
      widget_name = get_safe_item_name(widget_name);
      item_manager.get_first(packagesPath, widget_name, "widgets", ".js", function(err, html_file){
        if (err || !html_file){
    ...
    render_html_widget : function (packagesPath, widget_name, req, res){
      ...
      item_manager.get_first(packagesPath, widget_name, "widgets", ".css", function(err, css_file){
        if (!err && css_file){ //this is optional
          res.write("<style>");
          add_namespace(fs.readFileSync(css_file), res, widget_name);
          res.write("</style>");
        }
