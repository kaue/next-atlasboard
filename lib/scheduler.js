...
function Scheduler(jobWorker, widgets, options) {
...
Scheduler.prototype.scheduleNext = function(){
  var self = this;
  setTimeout(function(){
    self.start(self.jobWorker, self.widgets);
...
Scheduler.prototype.start = function() {
  ...
  function handleError(err) {
    ...
    self.jobWorker.config.interval = Math.min(self.jobWorker.config.original_interval / 3, 60000);
    ...
    if (sendError) {
      self.widgets.sendData({error: err, config: {interval: self.jobWorker.config.interval}});
    }
  }
  ...
  function handleSuccess(data) {
    ...
    self.jobWorker.config.interval = self.jobWorker.config.original_interval;
    if (!data) data = {};
    data.config = {interval: self.jobWorker.config.interval}; // give access to interval to client
    self.widgets.sendData(data);
  }

  self.jobWorker = ensureSafeJobWorkerConfiguration(self.jobWorker);
...
function ensureSafeJobWorkerConfiguration(jobWorker) {
  ...
  if (!jobWorker.config.original_interval) // set original interval so we can get back to it
    jobWorker.config.original_interval = jobWorker.config.interval;
