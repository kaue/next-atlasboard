module.exports = function() {
  var scheduler = {
    ...
    schedule: function(job_worker, widgets, dependencies) {
      ...
      try {
        task.call(job_worker, job_worker.config, dependencies, function(err, data){
          if (err){
            dependencies.logger.error('executed with errors: ' + err);
            ...
            if (job_worker.config.retryOnErrorTimes){
              ...
              if (job_worker.retryOnErrorCounter <= job_worker.config.retryOnErrorTimes){
                dependencies.logger.warn('widget with retryOnErrorTimes. attempts: '+ job_worker.retryOnErrorCounter);
                sendError = false;
            ...
            dependencies.storage.set('test', 'val');

            job_worker.retryOnErrorCounter = 0; //reset error counter on success
            dependencies.logger.log('executed OK');
        ...
        dependencies.logger.error('Uncaught exception executing job: ' + e);
      }
