  ...
  var scheduler = {
    ...
    schedule: function(job_worker, widgets) {
      ...
      if (!job_worker.config.interval){
        ...
        job_worker.config.interval = 1000; // minimum 1 sec
      }
      ...
      try {
        ...
        var cbCalled = false; // number of callbacks executed per job execution. It should be max 1.
        task.call(job_worker, job_worker.config, job_worker.dependencies, function(err, data){

          if (cbCalled) {
            job_worker.dependencies.logger.warn('WARNING!!!!: job_callback executed more than once for job ' +
                job_worker.widget_item.job + ' in dashboard ' + job_worker.dashboard_name);
          }
          cbCalled = true;
